...
PROGRESS_INDEX = elastic.INDEX_NAME_SL_PROGRESS
...
def insert_or_update_entities(entity: str, entity_type: str, entity_locations: list, entity_aliases: list ,entity_identifier: str):
    ...
    if document is None or not length_is_similar(document["_source"]["name"], entity):
        ...
        if "sl" not in body["information_source"]:
            body["information_source"].append("sl")
        if entity_identifier not in body["sl_record"]:
            body["sl_record"].append(entity_identifier)
